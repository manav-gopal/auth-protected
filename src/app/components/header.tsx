...
export default async function Header() {
  ...
  return (
    <header className="w-full border-b">
      <div className="flex h-6 items-end justify-end">
        <div className="flex h-3 items-center justify-end gap-4 pr-10">
          <Link
            href="#"
          ...
          {!user && (
            <>
              <Link href="/login" className="text-ct-dark-600 text-sm">
                Login
              </Link>
            </>
          )}
          {user && <AuthMenu userName={user?.name} />}
        </div>
      </div>
      <div className="flex w-full flex-col justify-between px-4 py-4 md:px-6">
        <div className="grid grid-cols-3 items-center justify-between max-md:grid-cols-2">
          <Link href="/" className="px-6 text-3xl font-bold">
            ECOMMERCE
          </Link>
          <nav className="hidden justify-center gap-8 md:flex md:font-bold">
            <Link href="#" className="text-base text-gray-600" prefetch={false}>
              Categories
            </Link>
            <Link href="#" className="text-base text-gray-600" prefetch={false}>
              Sale
            ...
            <Link
              href="#"
              className="text-nowrap text-base text-gray-600"
              prefetch={false}
            >
              New stock
            </Link>
          ...
          <div className="flex flex-row justify-end gap-10 px-5">
